...
Harden `backup` into a clear, reliable archive-and-manifest workflow that matches [backup-spec.md](docs/backup-spec.md), without expanding scope into retention, offsite storage, or encryption features.
...
- [src/cli.ts](src/cli.ts) exposes `backup create`, `backup list`, and `backup inspect`
- [src/commands/backup.ts](src/commands/backup.ts) now delegates `backup create` into a dedicated backup execution layer
- [src/lib/backup.ts](src/lib/backup.ts) now owns backup name resolution, destination path setup, metadata collection, archive creation, manifest writing, and artifact validation for `backup create`
- [src/lib/backups.ts](src/lib/backups.ts) owns backup path helpers, manifest read/write helpers, listing, and artifact validation
- [src/lib/mongo.ts](src/lib/mongo.ts) owns Mongo and SSH transport, metadata collection helpers, and archive creation
- [backup-spec.md](docs/backup-spec.md) now defines the intended backup behavior for this cleanup

### Current safety state

- backup metadata now excludes `system.*` collections
- archive and manifest validation exists through [src/lib/backups.ts](src/lib/backups.ts)
- backup create now has a phase-aware interruption and failure contract in [src/lib/backup.ts](src/lib/backup.ts)
...
- backup now has direct tests for backup creation, cleanup, interruption, output behavior, and command-surface behavior through [tests/backup.test.ts](tests/backup.test.ts)
...
[src/commands/backup.ts](src/commands/backup.ts) previously mixed:
...
Phase 2 resolves this by moving the create flow into [src/lib/backup.ts](src/lib/backup.ts). Later phases should continue to keep the command layer thin instead of moving orchestration back into it.
...
[README.md](README.md) now has a user-facing backup section with common workflows and full API coverage.

### Backup now has a spec and a dedicated execution layer

[backup-spec.md](docs/backup-spec.md) is now detailed enough to act as the behavior authority for backup cleanup, and [src/lib/backup.ts](src/lib/backup.ts) is now the correct place for later backup-create behavior changes.
...
Keep [src/commands/backup.ts](src/commands/backup.ts) responsible only for:
...
Introduce a dedicated backup runner, preferably in [src/lib/backup.ts](src/lib/backup.ts).
...
Keep [src/lib/backups.ts](src/lib/backups.ts) focused on backup artifact and manifest helpers:
...
Keep [src/lib/mongo.ts](src/lib/mongo.ts) as the low-level Mongo and SSH transport layer.
...
- [src/lib/backup.ts](src/lib/backup.ts) is now the canonical backup-create execution unit
- [src/commands/backup.ts](src/commands/backup.ts) should stay thin
- [tests/backup.test.ts](tests/backup.test.ts) currently covers only the successful create path and metadata shaping
...
- start from [src/lib/backup.ts](src/lib/backup.ts), not the command layer
...
- prefer testing [src/lib/backup.ts](src/lib/backup.ts) directly with injected dependencies before adding command-surface tests
...
- [src/commands/backup.ts](src/commands/backup.ts) now has a small dependency seam so command behavior can be tested without brittle module patching
...
- The next code change should begin in [src/lib/backup.ts](src/lib/backup.ts), not in [src/commands/backup.ts](src/commands/backup.ts).
...
- [src/lib/backups.ts](src/lib/backups.ts) should remain the artifact helper layer; avoid moving orchestration back into it while implementing later phases.
